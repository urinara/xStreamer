...
//					});
	...
	//	com.net.rtsp.Debug.println("RtspURLConnection.options()");
		...
		if (client != null) {
			...
			Debug.println("RtspURLConnection.options() response = " + responses);
	...
	//	System.err.println(" SETUP IS OK = "+isSuccessfullResponse());
		...
		//	System.err.println(" FND VALUE= "+(System.currentTimeMillis() - t0)+" SID="+sid);
			Debug.println("RtspURLConnection.setup() sid = " + sid);
		//	t0=System.currentTimeMillis();
			if (sid != null) {
				int i = sid.indexOf(';');
				if (i > 0) {
	...
	//	com.net.rtsp.Debug.println("RtspURLConnection.connect() describe supported =" +isSupportedMethod(DESCRIBE));
		...
		if (describNeeded) {
			...
			if (!(cnt instanceof ResponseContent) || !((ResponseContent) cnt).getContentType().equalsIgnoreCase("application/sdp")) {
				Thread.dumpStack();
				Debug.println("###### RtspURLConnection.connect() ERR CASE TYPE OF CNT = " + ((ResponseContent) cnt).getContentType());
				cleanup();
			}
	...
	protected synchronized void doCmd(String cmd, Object appData) throws IOException {
		...
		Debug.println("####### RtspURLConnection.doCmd("+cmd+") CSEQ B4 = "+getCseq());
		//Thread.dumpStack();
		if (!connected)
			throw new IOException("not connected");
		...
		try {
			...
			setCseq(r.doRequest(this, getClient(), requests, responses, getCseq()));
			Debug.println("####### RtspURLConnection.doCmd("+cmd+") CSEQ SET !!");
		} catch (IOException e) {
			//e.printStackTrace();
			...
			Debug.println("###### RtspURLConnection.doCmd("+cmd+") CSEQ AFTER = "+getCseq());
		}
	}
	...
	private void computeHostAndPort() {
		...
		if (useProxy) {
			...
			Debug.println("RtspURLConnection.computeHostAndPort() /*//*/*/*/*/ PORT = " + port);
			if (port == -1)
				this.port = PORT;
			else
	...
	private class ServerReplayer implements Observer {
		...
		public void update(Observable o, Object arg) {
			Debug.println("....update() server request received...");
...
//			}
	 ...
	 * 
	 */
	protected  class Client implements ClientProcessor {
		...
		public synchronized void sendRequest(RequestMessage request, ResponseMessage response) throws IOException {
			...
			Debug.println("Client.sendRequest() T-OUT =" + timeout);
			if (!timeout) {
		...
		private boolean parse(ResponseMessage responses, byte[] b) throws IOException {
			try {
			 ///	Thread.dumpStack();
				...
				Debug.println("Client.parse() IS NOT RTSP VALID!!!");
			} catch (IOException e) {
				throw e;
			}
			return false;
	...
	protected   class Server extends Observable implements ServerProcessor {
		...
		private void parse(byte[] b) throws IOException {
		 	Debug.println("Server.parse() "+new String(b));
					...
					if (lines.size() == 1) {// 1st line ==> method SO  METHOD ANNOUNCE [URL] RTSP/1.0
						...
						if (jj > 0) {
							String s1=l.substring(0, jj).trim(),
					 		...
					 		Debug.println("**Server.parse() "+s1+" / "+s2);
							h.add(s1,s2 );
						}
						else
							h.add(l, null);
